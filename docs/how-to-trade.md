...
Все примеры — **реальный BTCUSDT (Binance) в TradingView**, снимки от 20 сентября 2026,
индикатор [`MoneyForesight_v3.pine`](../MoneyForesight_v3.pine) v3.3 с настройками по умолчанию.
...
| **Линия Chandelier** | Ступенчатая линия: **голубая** под ценой, **фиолетовая** над ценой, **серая** — тренд не определён | Кто контролирует рынок: покупатели или продавцы? |
...
3. Свеча **закрывается выше обеих линий прошлой свечи**: и фиолетовой, что над ценой, и голубой,
   которая всё это время ждала снизу. Значит, цена ушла от недавнего минимума больше чем на 3 ATR —
   это уже не шум.
...
### Серая линия — неопределённость

После резкого импульса 22-свечной диапазон бывает шире 6 ATR, и обе линии оказываются «не на своей
стороне» цены: линия лонга выше цены, линия шорта ниже. Тренд в этот момент не определён, и
индикатор рисует линию **серой**.

Серая линия значит: старая граница тренда пробита, а новая ещё не подтверждена. Сделок по ней
открывать не стоит — ориентируйтесь на Rainbow и ждите, пока линия снова станет голубой или
фиолетовой. Обычно это 1–3 свечи, редко дольше.

> **Что изменилось в v3.3.** Раньше разворот засчитывался только при *пересечении* одной линии.
> После резкого импульса состояние застревало: на 1h 19–23 августа линия была фиолетовой, но
> стояла под ценой весь рост с 68 до 77 тысяч. Таких свечей на BTC было 5–11% истории. Теперь
> разворот требует закрытия за обеими линиями, и такие эпизоды занимают около 2% свечей —
> это и есть серая зона неопределённости.
...
5. **Линия ведёт тренд ступенями без сбоев до 28 августа**, когда цена закрылась под ней и линия
   встала над ценой.
...
- **Пока линия серая** — тренд не определён после резкого хода.
...
|---|---|---|---|
| BUY / SELL (смена тренда) | 111 | **0.94** | 0.75 |
| Ретест (треугольник) | 261 | **1.33** | 0.98 |
| Повторный вход (ромб) | 53 | **1.45** | 1.20 |
...
3. Ретесты идут по ходу тренда 20 и 21 августа — вход по зоне в уже подтверждённом движении.
4. Rainbow ярко-зелёная — импульс сильный.
5. 22 августа цена закрылась под линией: разворот, линия встала над ценой.

### 15m

![BTC 15m, 16–19 сентября 2026](img/guide/btc-15m-trend.jpg)

1. 16 сентября — флэт: линия часто меняет цвет, входов нет.
2. 17 сентября — BUY.
3. 18 сентября, 09:30 — ретест у голубой зоны.
4. 13:30 — второй ретест, после него рывок к 81 тыс.
5. Rainbow ярко-зелёная — импульс в силе.
6. 19–20 сентября Rainbow желтеет, рост встал: цена упёрлась в канал 81.5–82.9 тыс.

### 5m

![BTC 5m, 17–18 сентября 2026](img/guide/btc-5m-trend.jpg)

1. Флэт: линия почти плоская, Rainbow оранжевая — не торговать.
2. 03:10 — BUY.
3. Дальше линия ведёт тренд ступенями, сбоев нет.
4. 11:10–13:30 — настоящий откат: линия развернулась и встала **над** ценой.
5. 13:35 — ретест, после него рост до 81.2 тыс.
6. Rainbow позеленела уже после рывка: на 5m её запаздывание — около 2 часов.
...
2. **14 сентября — FLIP**: линия развернулась, сделка закрыта досрочно.
3. **15 сентября — SELL** на 76.5 тыс. Но пришёл он **всего в ~250 долларах над зелёным каналом
   поддержки** 74.9–76.3 тыс., а тейк стоял под этим каналом.
4. Цена **отскочила от поддержки**, и 18 сентября шорт выбило по стопу. Это та самая ошибка
   «продавать прямо над поддержкой» из раздела 5.
5. **18 сентября — вход в лонг по ретесту** на 80 726. Свечи при этом красные: SELL ещё действует,
...
6. **Красный канал 81.5–82.9 тыс.** лежит между входом и тейком — первое препятствие. Через два дня
   после входа цена так и стоит под ним: сделка открыта, до тейка далеко.
...
1. **Тренд: 381 отбой и 160 пробоев** — зона удержала цену в 70% касаний, средний ход после
   отбоя 1.45%.
2. **Боковик: 66%**, и средний ход после отбоя меньше — 1.02%. Во флэте отбои случаются почти так
   же часто, но движение после них короче. Поэтому торговать отбои выгоднее в тренде.
3. **Виртуальные сделки: 32 тейка, 33 стопа, 40 выходов по FLIP.** Winrate 49% считается только из
...
4. **Таблица** отсортирована по силе, строка 1 — самый сильный канал. Столбцы: номер, тип, нижняя и
   верхняя граница.
5. **Красный канал на графике — строка 4** «Resistance» 81 479–82 850: сопротивление прямо над ценой.
   Ещё 19 сентября цена была внутри него, и строка была серой «In channel».
...
- **Входить, пока линия серая** — тренд в этот момент не определён.
